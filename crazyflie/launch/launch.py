...
def parse_yaml(context):
    ...
    for key, value in crazyflies['robots'].items():
        rtype = crazyflies['robot_types'][value['type']]
        if value['enabled'] and \
            ((fileversion == 1 and rtype['motion_capture']['enabled']) or \
            ((fileversion >= 2 and rtype['motion_capture']['tracking'] == "librigidbodytracker"))):
            motion_capture_params['rigid_bodies'][key] = {
                'initial_position': value['initial_position'],
                'marker': rtype['motion_capture']['marker'],
                'dynamics': rtype['motion_capture']['dynamics'],
            }
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # NEW: su_interface on/off switch
        DeclareLaunchArgument('su_interface', default_value='True'),
        ...
        # NEW: run `ros2 run crazyflie_examples su_interface`
        Node(
            condition=LaunchConfigurationEquals('su_interface', 'True'),
